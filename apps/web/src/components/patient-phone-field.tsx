...
export function PatientPhoneField({
  ...
  const conflictPatient = data?.conflict ? (data.patient ?? null) : null;

  useEffect(() => {
    onConflictChange?.(conflictPatient);
  }, [conflictPatient, onConflictChange]);

  const showConflict = Boolean(conflictPatient);

  return (
    <div className="space-y-2">
      <Label required>{t("patients.phone")}</Label>
      <Input
        className={cn("ltr-nums", showConflict && "border-destructive focus-visible:ring-destructive")}
        value={value}
        onChange={(e) => onChange(e.target.value)}
        aria-invalid={showConflict}
        aria-describedby={showConflict ? "patient-phone-conflict" : undefined}
      />
      {showConflict && conflictPatient ? (
        <p id="patient-phone-conflict" className="text-sm text-destructive">
          {phoneConflictMessage(conflictPatient, t, i18n.language)}{" "}
          <Link to={`/patients/${conflictPatient.id}`} className="font-medium underline underline-offset-2">
